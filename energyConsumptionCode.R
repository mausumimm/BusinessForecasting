...
#Data Loading
consumption.df <-read_excel('IEloadconsumptiondataset.xlsx', sheet = "Final")
...
#Time Series
Consumption.ts<-ts(consumption.df$`Average of load`,start = c(2015,1),end=c(2020,7),frequency=12)
Consumption.ts
plot(Consumption.ts, xlab = ('Year'), xlim = c(2015, 2024), ylab = ('Total Energy'), ylim = c(2000, 3900), main = ('Energy Consumption'))
...
train.ts <- window(Consumption.ts, start = c(2015,1), end = c(2015, ntrain))
valid.ts <- window(Consumption.ts, start =c(2015, ntrain +1), 
                   end = c(2015, ntrain+nvalid))
ntrain
train.ts
valid.ts
plot(train.ts, xlab = ('Year'), xlim = c(2015, 2024), ylab = ('Total Energy'), ylim = c(2000, 3900), main = ('Training and Validation Set:Energy Consumption'))
lines(valid.ts, col='red')
lines(c(2019.4,2019.5), c(0, 3800), col='green')
text(2017, 3900, "Training")
text(2020, 3900, "Validation")
#Moving Average Line
...
lines(c(2019.4,2019.5), c(0, 4500), col='green')
lines(c(2020.5, 2020.5), c(0, 4500), col='green')
text(2017, 4400, "Training")
text(2020, 4400, "Validation")
text(2023, 4400, "Test")
predict(forecasting)
###############
